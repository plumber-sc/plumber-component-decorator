...
namespace Plugin.Plumber.Component.Decorator.Pipelines.Blocks
{
    ...
    public class GetComponentViewBlock : PipelineBlock<EntityView, EntityView, CommercePipelineExecutionContext>
    {
        ...
        public async override Task<EntityView> Run(EntityView entityView, CommercePipelineExecutionContext context)
        {
            ...
            // Get a list of all the component types that have been registered for this entity
            var viewableComponentTypes = GetViewableComponentTypes(context, request, components);
        ...
        private IEnumerable<Type> GetViewableComponentTypes(CommercePipelineExecutionContext context, EntityViewArgument request, IList<Sitecore.Commerce.Core.Component> components)
        {
            var knownComponentTypesInEntity = this.commander.GetAllComponentTypes().Where(type => components.Any(comp => type.IsInstanceOfType(comp)));

            var applicableComponentTypes = this.commander.GetApplicableComponentTypes(request.Entity, request.ItemId, context.CommerceContext);

            var viewableComponentTypes = knownComponentTypesInEntity.Union(applicableComponentTypes);
            return viewableComponentTypes;
        }
    }
}